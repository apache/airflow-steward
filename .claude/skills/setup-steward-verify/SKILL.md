...
[`setup-isolated-setup-verify`](../setup-isolated-setup-verify/SKILL.md), which
...
setup wired (setup-isolated-setup-verify catches that), or have the
...
- ⚠ for unfilled fields that are convenience-only or apply
  ...
  the security-cve-allocate skill), Gmail / PonyMail OAuth flags
...
- `+` prefix = the submodule's `HEAD` is *ahead of* the SHA
  ...
  in the parent (`setup-steward-upgrade` skill mentions this
...
Recommend
[`setup-isolated-setup-verify`](../setup-isolated-setup-verify/SKILL.md) as
